...
// Add spacebar functionality to launch multiple random shells instantly
function handleKeydown(event) {
    // Spacebar (keyCode 32) - Launch multiple random shells
    if (event.keyCode === 32) {
        event.preventDefault(); // Prevent scrolling
        launchMultipleShells(); // Lance plusieurs feux d’un coup
    }
    // P = Pause
...
// Nouvelle fonction : lance plusieurs shells simultanément
function launchMultipleShells(count = 6) {
    // Si tu veux ignorer le "isRunning()", commente la ligne ci-dessous
    if (!isRunning()) return;
    ...
    for (let i = 0; i < count; i++) {
        ...
        const x = getRandomShellPositionH();
        const y = getRandomShellPositionV();

        shell.launch(x, y);

        // Son de lancement (tu peux le déplacer hors de la boucle si tu veux un seul son)
        soundManager.playSound('lift');
    }
}

// Remplace l’ancien listener par le nouveau
